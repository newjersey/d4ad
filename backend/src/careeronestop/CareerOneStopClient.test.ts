import {GetOccupationDetailPartial, GetOpenJobsCount} from "../domain/types";
...
describe('CareerOneStopClient', () => {
  ...
  beforeEach(() => {
    ...
    getOpenJobsCount = CareerOneStopClient("wwww.some-cool-url.com", "FAKE-USERID", "FAKE-AUTH-TOKEN");
  });
  ...
  it('returns -1 if the call to career one stop fails', async () => {
    mockedAxios.get.mockRejectedValue({ })
    const jobsCount = await getOpenJobsCount("15-1134")
    expect(jobsCount).toEqual(-1)
  })
})